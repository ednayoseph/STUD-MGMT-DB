...
def update_student():
    ...
    if ret==0:
        conn=connection()
        cur=conn.cursor()
        cur.execute("UPDATE STUDENTS SET NAME=?,ROLL_NO=?,BRANCH=?,PHONE_NO=?,FATHER=?,ADDRESS=?,STATUS=? where ROLL_NO=?",(student_name.get(),int(roll_no.get()),branch.get(),int(phone.get()),father.get(),address.get(),int(roll_no.get()),status.get()))
        conn.commit()
        conn.close()
...
if __name__=="__main__":
    ...
    e7=Entry(root,textvariable=status)
